...
static void initialise_ui(void) {
	...
	timer_layer = text_layer_create(GRect(0, 55, 117, 61));
	...
	text_layer_set_font(timer_layer, s_res_leco_42_numbers);
...
static void set_times_from_store_and_display() {
...
static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
	if(mode == MODE_RUNNING_WORK || mode == MODE_RUNNING_PAUSE){
		...
		mode = MODE_PAUSED;
		set_times_from_store_and_display();
	}else{
		m = worktime;
		s = 0;
		mode = MODE_RUNNING_WORK;
